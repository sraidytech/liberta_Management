...
/**
 * 🚀 ADMIN SYNC TEST WITH REFERENCE-BASED DUPLICATE CHECK
 * This replicates the exact logic of OrdersController.syncAllStores()
 * BUT fixes the duplicate check to use REFERENCE instead of just ecoManagerId
 * 
 * ISSUE: Current logic checks: WHERE ecoManagerId = '20525'
 * PROBLEM: This finds NATU20525 when looking for ALPH20525
 * 
 * SOLUTION: Check by reference: WHERE reference = 'ALPH20525'
 * This ensures each store's orders are checked independently
 */
async function testAdminSyncWithReferenceCheck() {
  console.log('🚀 ADMIN SYNC TEST WITH REFERENCE-BASED DUPLICATE CHECK');
  console.log('=' .repeat(80));
  console.log('This test replicates the EXACT logic of the admin "Sync New Orders" button');
  console.log('BUT fixes the duplicate check to use REFERENCE instead of just ecoManagerId');
  console.log('');
  ...
  console.log('');
  console.log('✅ SOLUTION: Check by reference: WHERE reference = "ALPH20525"');
  ...
  try {
    ...
    for (const apiConfig of activeConfigs) {
      ...
      const result = await testStoreWithReferenceCheck(apiConfig);
      results.push(result);
...
async function testStoreWithReferenceCheck(apiConfig: any): Promise<TestResult> {
  ...
  try {
    ...
    for (const ecoOrder of allOrders) {
      try {
        ...
        // 🟢 NEW LOGIC: Check by reference (FIXED BEHAVIOR)
        const existingOrderNewLogic = await prisma.order.findUnique({
          where: { reference: ecoOrder.reference }
        });
        ...
        if (wouldBeSkippedByOldLogic && !wouldBeSkippedByNewLogic) {
          ...
          console.log(`   ✅ NEW LOGIC: Would process because reference ${ecoOrder.reference} doesn't exist`);
        }
    ...
    while (pagesScanned < maxPages && scannedPastLastOrder < 30) {
      ...
      if (missedOrders.length > 0) {
        ...
        for (const order of missedOrders) {
          // 🟢 Use reference-based check instead of ecoManagerId
...
function generateCompleteAnalysis(results: TestResult[]) {
  ...
  console.log('\n✅ PROPOSED SOLUTION:');
  console.log('   - Change sync to check: WHERE reference = "ALPH20525"');
  ...
  if (totalOrdersSkippedByOldLogic > 0) {
    ...
    console.log('   The fix is simple: change the duplicate check from ecoManagerId to reference.');
  }
  ...
  console.log('   const existingOrder = await prisma.order.findUnique({');
  console.log('     where: { reference: ecoOrder.reference }');
  console.log('   });');
  console.log('');
  console.log('✅ This simple change will fix the duplicate order issue!');
}

// Run the test
testAdminSyncWithReferenceCheck().catch(console.error);