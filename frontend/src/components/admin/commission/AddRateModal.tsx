...
export function AddRateModal({ agents, products, onClose, onSave }: AddRateModalProps) {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/v1/commissions/agents/${formData.agentId}/products/${encodeURIComponent(formData.productName)}/rates`, {
        method: 'POST',
        headers: {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-100">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-bold text-gray-900">
              {language === 'fr' ? 'Nouveau Taux Produit' : 'New Product Rate'}
            </h3>
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            >
              <option value="">{language === 'fr' ? 'Sélectionner un agent' : 'Select an agent'}</option>
              {agents.map((agent) => (
        ...
        <div className="p-6 border-t border-gray-100 flex justify-end space-x-3">
          ...
          <Button
            onClick={handleSave}
            disabled={loading || !formData.agentId || !formData.productName}
          ...
          >
            ...
            <span>{loading ? (language === 'fr' ? 'Enregistrement...' : 'Saving...') : (language === 'fr' ? 'Enregistrer' : 'Save')}</span>
          </Button>
        </div>
      </div>
    </div>
